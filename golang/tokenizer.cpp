...
long long segmentPointer(const char *go_text, bool for_transforming, int tokenize_option, bool keep_puncts, int text_length) {
	...
	res_pointer[0] = reinterpret_cast<int64_t>(text);
	res_pointer[1] = reinterpret_cast<int64_t>(text ->data());
	res_pointer[2] = (int64_t) ranges->size();
	res_pointer[3] = reinterpret_cast<int64_t> (ranges->data()); // pointer to raw data array inside vector
	res_pointer[4] = reinterpret_cast<int64_t> (ranges);	 // pointer to actual vector
	res_pointer[5] = (int64_t) space_positions->size();
	res_pointer[6] = reinterpret_cast<int64_t> (space_positions->data());
	res_pointer[7] = reinterpret_cast<int64_t> (space_positions);
